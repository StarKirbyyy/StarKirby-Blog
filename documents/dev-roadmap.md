...
- [ ] `generateStaticParams()`：SSG 预生成所有文章页
- [ ] `generateMetadata()`：动态生成 SEO title / description / og tags
- [ ] 文章标题、元信息（日期、阅读时长、标签）渲染
- [ ] MDX 正文渲染（含自定义组件映射）
- [ ] `@tailwindcss/typography` prose 排版样式
- [ ] 代码块：语言标识 + 复制按钮
- [ ] 目录（TOC）组件：右侧 sticky（桌面） / 隐藏（移动端）

### 3.2 文章列表页 `/posts`
- [ ] 文章列表渲染（标题、日期、摘要、标签、阅读时长）
- [ ] 按日期倒序排列
- [ ] 分页组件（确认方式后实现）

### 3.3 首页 `/`
- [ ] Hero 区块：头像、博主名、简介、社交链接
- [ ] 最新文章预览（取最新 5 篇）
- [ ] 「查看全部文章」入口
...
- [ ] 关于页 `/about`：头像、简介（MDX）、技能标签、联系方式
- [ ] 项目展示页 `/projects`（若需要）：项目卡片列表
- [ ] 自定义 404 页面
